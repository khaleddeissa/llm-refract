...
# Copy manifests first to improve dependency caching.
...
RUN apt-get update \
    ...
    && useradd --uid 10001 --gid refract --no-create-home refract \
    && mkdir -p /data \
...
# Verify that the server is ready to accept requests.
HEALTHCHECK --interval=10s --timeout=3s --start-period=10s \
    CMD curl -fsS http://127.0.0.1:8000/v1/ready || exit 1
